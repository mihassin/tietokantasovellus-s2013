<?php 
	...
	function getMaterials() {
        ...
        function getOrders() {
		$querystr = "SELECT id, user_id, address, order_time, total_price FROM orders WHERE delivered=FALSE;";
		return $querystr;
	}

        function getCartByUid($uid) {
        	$querystr = "SELECT cart_map.id as id, products.name as name, cart_map.amount as kpl, cart_map.price as price, cart_map.added_mats as mats FROM products, cart_map WHERE products.id = cart_map.product_id AND cart_map.ordered = FALSE AND cart_map.user_id = {$uid};";
        	return $querystr;
        }
